// lib/api/encrypted-client.ts - FIXED REFRESH TOKEN LOGIC
...
class EncryptedApiClient {
    ...
    this.client.interceptors.response.use(
      ...
      async (error) => {
        ...
        if (error.response?.status === 401 && !originalRequest._retry) {
          // Check if we recently refreshed to avoid loops
          const now = Date.now();
  ...
  // TOKEN REFRESH - FIXED TO HANDLE ENCRYPTED RESPONSE
  private async refreshAccessToken(refreshToken: string) {
    try {
      ...
      if (data?.__payload) {
        const { __checksum, __payload } = data;

        if (__checksum) {
          const calculatedChecksum = this.generateChecksum(__payload);
          if (calculatedChecksum !== __checksum) {
            throw new Error('Token refresh response integrity check failed');
          }
        }
