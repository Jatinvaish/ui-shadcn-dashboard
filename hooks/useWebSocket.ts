...
export const useWebSocket = (token: string | null, userId: number | null): UseWebSocketReturn => {
  ...
  const replyInThread = useCallback(
    (parentMessageId: number, content: string, channelId: number, mentions?: number[]) => {
      if (!socketRef.current?.connected) return;
  ...
  const sendFileMessage = useCallback(
    async (data: {
      ...
      return new Promise((resolve) => {
        console.log("📎 Sending file message:", data);

        socketRef.current!.emit("send_file_message", data, (response: any) => {
